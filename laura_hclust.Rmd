...
```{r}
set.seed(123)
dist_mat <- dist(feature_mat_filtered, method = 'binary')
...
```{r}
# For now set cluster A as cluster 2 (biggest cluster) and cluster B as all other clusters
cluster.assignments <- ifelse(cut_ward == 2, "A", "B")
```

```{r}
# Multi-class
cluster.assignments <- cut_ward
```


```{r}
cut_ward2 <- cutree(hclust_ward, h = 80)
cluster.assignments.2 <- cut_ward2
```

...
plot(hclust_ward, main = "Mutation Hierarchical Clustering Dendogram")
...
# categorical independent variable `cluster groups`, and effectively create a formula
...
plotMA(res, ylim = c(-3, 3), 
       main = "Log fold changes between mutation analysis clusters")
```
...
colnames(sample.order.df) <- c(cluster)
...
```{r}
# Plot the log2 fold expression of the top 17 most mutated genes
ens.cleaned <- sapply(strsplit(rownames(counts), 
                                      "\\."), "[", 1)

genes.sym = mapIds(org.Hs.eg.db,
                    keys=ens.cleaned, 
                    column="SYMBOL",
                    keytype="ENSEMBL",
                    multiVals="first")

mut.genes <- which(genes.sym %in% colnames(feature_df_filtered))

sampleMatrix <- assay(vst)[mut.genes,]
colnames(sampleMatrix) <- colnames(counts)
rownames(sampleMatrix) <- genes.sym[mut.genes]
...
pheatmap(sampleMatrix,
         ...
         cluster_cols=FALSE,
         annotation_col=annotation.df,
         main = "Expression of most highly mutated genes across clusters")
```
While expression analysis showed that genes are significantly differentially expressed across clusters, the genes that were identified as most highly mutated across all three clusters do not show differential expression. 
...
```
6 pathways were identified as downregulated in cluster 2 compared to cluster 3. These pathways included the cell cylce, natural killer cell mediated cytotoxicity, IgA production, and antigen processing.

```{r}
head(kegg.res$greater, 20)
```

19 pathways were identified as significantly upregulated in cluster 2 compared to cluster 3. Some notable clusters include the MAPK signaling pathway, mTOR signaling pathway, mRNA surveillance pathway, the peroxisome pathway, and other metabolic pathways.

Observation: The MAPK signaling pathway is significantly upregulated in cluster 2 compared to cluster 3. Cluster 3's third highest frequency mutated gene is MAPK, which likely means that cluster 3 tumours often have MAPK loss of function mutations.
